...
A .NET command-line tool for extracting and analyzing assemblies from .NET MAUI Android Apk files built with .NET 8 or above.

## Overview

MauiAssemblyExtractor is a cross-platform console application designed to extract .NET managed assemblies embedded within Android APK packages created with Microsoft's MAUI framework targeting **.NET 8 or above**. MAUI applications bundle .NET assemblies in files such as `assemblies*.blob`, `libassembly-store.so`, or `libassemblies.blob.so`. This tool automates the entire extraction process, including:
...
2. **MAUI APK built with .NET 8 or above** - This tool is compatible only with APK files built targeting .NET 8 or later versions
